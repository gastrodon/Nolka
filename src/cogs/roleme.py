...
class Roleme(commands.Cog):
    ...
    async def _add_self_role(self, ctx, roles):
        ...
        valid = [
            ...
            if role.id in ctx.bot.cache[str(ctx.guild.id)].get(
                "self_roles", [])
        ]
    ...
    async def _remove_self_role(self, ctx, roles):
        ...
        valid = [
            role for role in roles
            if role.id in ctx.bot.cache[str(ctx.guild.id)].get(
                "self_roles", []) and role in ctx.author.roles
        ]
        ...
        await ctx.send(embed = await Macro.send(
            f"Removed from you the roles {', '.join(valid)}"))
    ...
    async def roleme(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            await ctx.send(embed = await Macro.send(
                f"This guild doesn't have any self assignable roles"))
    ...
    @roleme.command(pass_context = True,
                    name = "deny",
                    aliases = ["block", "disallow"])
    @commands.has_permissions(manage_roles = True)
    async def disallow_self_role(self, ctx,
                                 *roles: typing.Union[discord.Role]):
        """
        ...
        await ctx.send(embed = await Macro.send(
            f"Disallowed the self roles {', '.join([str(role) for role in roles])}"
        ))
