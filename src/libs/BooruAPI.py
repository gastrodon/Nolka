...
class Booru:
    ...
    def __init__(self, ctx, message, tags):
        ...
        self.cachedir = os.path.dirname(
            os.path.realpath(__file__)) + "/__cache"
    ...
    async def no_posts(self):
        await self.message.edit(embed = await Macro.send("No posts were found")
                                )
...
class Gel(Booru):
    ...
    async def edit_message(self):
        ...
        if self.info:
            tags = [
                tag.replace("_", "\_")
                for tag in post["tags"].strip().split(" ")
            ]
        ...
        url = post["file_url"]
        self.url = url
        source = f"[Source]({post['source'].split(' ')[0]})" if post[
            "source"] else "No source"
...
class Derpi(Booru):
    def __init__(self, *args, **kwargs):
        ...
        self.queryStrings = {
            "q":
            f"{','.join([a.replace('_', ' ') for a in self.tags])}, -animated",
            "page": self.page + 1,
    ...
    async def edit_message(self):
        ...
        try:
            rating = list(
                filter(lambda x: x in ["explicit", "questionable", "safe"],
                       tags))[0][0].upper()
        except IndexError:
            rating = "?"
        if self.info:
        ...
        self.url = url
        source = f"[Source]({post['source_url'].split(' ')[0]})" if post[
            "source_url"] else "No source"
        embed = await Macro.Embed.image(url)
        embed.title = f"{self.index + 1} of {self.total} results | Page {self.page + 1} | Rating: {rating}"
        embed.description = source
        return await self.message.edit(embed = embed)
...
class E621(Booru):
    def __init__(self, *args, **kwargs):
        ...
        self.queryStrings = {"tags": f"{' '.join(self.tags)} -animated"}
