...
class Workers:
    ...
    async def _notify(ctx, user, mode, reason):
        try:
            ...
            await user.dm_channel.send(embed = await Macro.Embed.infraction(
    ...
    async def _get_mutable_role(ctx):
        ...
        if await ctx.bot.flag_check(guild):
            ...
            await ctx.bot.send_debug(
                "cleared flag and updated scope for guild {guild.name}")
    ...
    async def _update_mute_scope(guild):
        try:
            ...
            existing = discord.utils.get(guild.roles, name = "muted")
            role = existing if existing else await guild.create_role(
                name = "muted")
    ...
    async def mute_timer(ctx, user, duration):
        ...
        await ctx.send(embed = await Macro.send(f"Muted {user.name}"))
